# bridge pull/sync on startup — design
...
`bridge <name>` cd's into a repo and launches Claude Code. The local checkout may be hours or days behind the remote — work then begins on a stale tree, sometimes producing duplicate or conflicting changes against commits that already exist upstream.
...
```
bridge: pulled <short-old>..<short-new> on <branch>
```
...
All non-silent skip messages are printed in yellow (`\033[33m … \033[0m`) prefixed `bridge:` so they are visible but not alarming. They go to stderr so they don't pollute any stdout consumer.
...
**No** new env var, no settings file. If a user wants permanent opt-out, they alias `bridge` to `bridge --no-sync`.
...
**Tab completion:** add `--no-sync` to the flags list in `_bridge`.

## Integration point

A new function `_bridge_sync` is added in `shell/bridge.sh`, defined immediately above `_bridge_launch`.

It is called from `_bridge_launch` immediately after `cd "$_BRIDGE_BASE/$sel"` and before the MRU update — so:
...
The reattach-skip check uses the *same* tmux session-name derivation as `_bridge_launch` so the two stay in sync. To avoid drift, the session name is computed by a small shared helper `_bridge_tmux_session_name "$repo" "$worktree"` used both by `_bridge_sync` (skip check) and `_bridge_launch` (session create/attach).
...
## Argument-parser changes in `bridge()`

- Add local: `_BRIDGE_NO_SYNC=0` alongside the other locals at the top.
- Add case branch: `--no-sync) _BRIDGE_NO_SYNC=1; shift ;;`
- Add help line as shown above.

## `_bridge_launch` changes

Right after `cd "$_BRIDGE_BASE/$sel" || return` and before the `mru` block:

```bash
_bridge_sync "$(basename "$sel")" "$worktree"
```

Refactor `_bridge_launch` to use the new `_bridge_tmux_session_name` helper instead of its inline derivation, so both call sites are consistent.
...
1. **Up-to-date** — `bridge config` from a clean tree at the tip of `main` → silent, claude launches.
2. **Behind** — reset to a prior commit, `bridge config` → see "pulled abc..def on main" line, then claude launches with current tip.
3. **Dirty** — `touch x; bridge config` → see "dirty working tree, skipping sync" warning, claude launches.
...
5. **Detached HEAD** — `git checkout HEAD~1` then `bridge config` → see "detached HEAD, skipping sync" warning.
6. **No upstream** — `git checkout -b throwaway`, `bridge config` → see "no upstream for throwaway, skipping sync" warning.
7. **`--no-sync`** — `bridge --no-sync config` → silent, no fetch.
8. **Tmux reattach** (skip if not in SSH context) — start a session, detach, re-run → no fetch second time.
9. **Lint** — `bash-language-server` (LSP) clean over `shell/bridge.sh`.

No unit-test framework exists for this script; verification is by manual smoke + LSP.

## Out of scope (could be added later)

- Background fetch with a `bridge: behind by N` indicator if the user prefers a non-mutating default.
- A `bridge --sync-all` housekeeping command that walks every cloned repo and runs the same sync.
