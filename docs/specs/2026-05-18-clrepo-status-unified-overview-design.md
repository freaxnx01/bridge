# clrepo `--status` — unified session overview — design

**Date:** 2026-05-18
**Component:** `clrepo.sh` — rewrite of `_clrepo_slot_status`, extension of `_clrepo_tmux_session_defaults`, deprecation of `_clrepo_slot_status_rc`
**Issue:** [#1](https://github.com/freaxnx01/clrepo/issues/1)
**Followup blocked by this:** [#2](https://github.com/freaxnx01/clrepo/issues/2)

## Problem

`clrepo --status` today only lists clrepo-managed *slots* (`s0..sN`), which correspond to Telegram channel/bot sessions. Claude sessions started in other clrepo modes are invisible:
...
`clrepo --status` becomes the single overview of all clrepo-managed Claude sessions on this host — slot or not, RC-active or not, tmux or foreground. Issue #2 (interactive connect mode) is then a thin layer on top of the unified row set.

## Non-goals

- **No broader discovery.** Hand-rolled tmux sessions running `claude`/`opencode` outside clrepo are explicitly *not* surfaced. Discovery is scoped to sessions clrepo itself created. This is the conservative choice locked in during brainstorming; broader scans (pane-cmd heuristics) are out of scope.
...
- **No backfill of pre-existing tmux sessions.** Sessions launched by an older clrepo version don't carry the tags; they remain invisible to the new code path. Acceptable — they were invisible to `--status` before this change too.
...
At every site where `clrepo` creates a tmux session, we set tmux user-options on that session immediately after `tmux new-session`. The set is centralised in `_clrepo_tmux_session_defaults` (`clrepo.sh:1745`), which is already called from every create site, so this is a single function edit plus an argument-passing pass at the call sites.

New `_clrepo_tmux_session_defaults` signature:

```sh
_clrepo_tmux_session_defaults <session> <repo> <worktree> <kind> <slot> <pid>
```
...
- `pid` — pid of the in-pane process, read once via `tmux display-message -t <session> -p '#{pane_pid}'` after the session has spawned. This is the same mechanism the slot path already uses (`clrepo.sh:1962`), so synthetic tmux rows and slot rows carry the same kind of pid — RC lookup behaves identically across both.

The function sets the corresponding user-options:

```
@clrepo-repo       <repo>
@clrepo-worktree   <worktree>
@clrepo-kind       <kind>
@clrepo-slot       <slot>
@clrepo-pid        <pid>
```
...
| Site | Kind |
|---|---|
| VS Code path (`clrepo.sh:1851`) | `code` |
| opencode path (`clrepo.sh:1882`) | `opencode` |
| `--no-channel` path (`clrepo.sh:1904`) | `no-channel` |
| Slot path (`clrepo.sh:1937`) | `slot` |

All four already invoke `_clrepo_tmux_session_defaults "$session"` — we extend each invocation with the additional args. Call sites that don't spawn tmux (foreground modes) are unaffected.

### Enumeration

`_clrepo_slot_status` produces its row list from the **union** of two sources:

1. **`slots.json` rows** — current behavior, post `_clrepo_reconcile_slots`. Each row carries `slot`, `repo`, `worktree`, `pid`, `started_at`, `session` (tmux name or empty).
2. **Tmux-tagged sessions** — every `tmux list-sessions` entry where `@clrepo-repo` is set.
...
Tab-separated for parsing safety. Rows with empty `@clrepo-repo` are dropped (untagged or non-clrepo sessions).
...
No new logic. Existing `_clrepo_reconcile_slots` continues to prune dead `slots.json` rows. Tmux-tagged synthetic rows don't need pruning: if the tmux session is gone, `tmux list-sessions` doesn't return it.
...
RC lookup uses the same logic as today's `_clrepo_slot_status_rc`: for slot rows, read `~/.claude-s<N>/sessions/<pid>.json` and pick up `bridgeSessionId`. For synthetic `no-channel` rows, read `~/.claude/sessions/<pid>.json` (their shared config dir). For `code`/`opencode` rows, no Claude session file exists, so RC stays `—`.
...
```
SLOT  KIND        REPO                 STARTED        PID      TMUX                 BOT                          RC
s0    slot        clrepo               1h05m ago      12345    clrepo               (admin bot)                  ✓
...
- Bump `_CLREPO_VERSION` minor (new feature).
...
All edits land in `clrepo.sh`:

- `_clrepo_tmux_session_defaults` — signature change + new option writes.
- Four call sites that invoke the helper — pass the new args.
- `_clrepo_slot_status` — full rewrite for the unified table + footer.
- `_clrepo_slot_status_rc` — collapse to alias.
...
1. **Coverage** — start one slot session, one `--no-channel` SSH session (so it runs in tmux), one hand-rolled tmux session running `claude` outside clrepo. `clrepo --status` shows exactly the first two; the hand-rolled one stays invisible.
...
8. **Old-version compatibility** — manually launch a `--no-channel` SSH session via the *pre-change* `clrepo.sh`, switch to the new binary, run `--status`. Row is invisible (no tags). Acceptable per non-goals; documented in CHANGELOG.
