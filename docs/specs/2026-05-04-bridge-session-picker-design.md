# bridge session picker (`--attach`) — design

**Date:** 2026-05-04
**Component:** `shell/bridge.sh` — new function `_bridge_attach_pick`, refactor of `_bridge_slot_status` reconcile step
**Issue:** [#18](https://github.com/freaxnx01/config/issues/18)

## Problem

To reattach to a running Claude Code session today, the user types the repo name (and worktree, if any): `bridge <repo> [-w <wt>]`. The reattach detection at `shell/bridge.sh:1313` does the right thing once the args land. The friction is that the user has to remember which slot is running which repo — `bridge --status` already shows the answer in a table — and then retype it.
...
- **No pre-filtering by repo.** `bridge --attach <repo>` is rejected. The picker is small (≤ 6 rows by default) and fzf does live fuzzy filtering for free.
- **No new state.** The picker reads the same `slots.json` that `--status` does. No MRU-of-attaches, no separate cache.
- **No copilot/VS Code support.** Copilot mode skips slot recording (`shell/bridge.sh:1253-1275`); VS Code skips tmux entirely (`shell/bridge.sh:1244-1248`). Neither produces an attachable tmux session backed by a slot record.
...
New flag, parsed in `bridge()` next to `--status` and `--free`:

```
-a|--attach     fzf picker over live sessions (--status rows); reattach to selection
```

Help text gets one line under the slot-management group in the heredoc at `shell/bridge.sh:1484-1520`. Tab-completion gets `--attach` and `-a` added to the flags list at `shell/bridge.sh:1698`.
...
```
$ bridge --attach -w doc
bridge: --attach takes no other flags (got: -w). Run `bridge <repo> -w <wt>` to launch.

$ bridge --attach config
bridge: --attach takes no positional args. Run `bridge <repo>` to launch.
```

### Function: `_bridge_attach_pick`

Lives next to `_bridge_slot_status` in the slot-helpers block (around `shell/bridge.sh:1080-1150`). Does five things:

1. **Reconcile.** Calls `_bridge_reconcile_slots` (extracted from `_bridge_slot_status`, see below) so dead slots are pruned from `slots.json` before reading.
...
4. **Count-branch:**
   ...
   | Live count | Action |
   |---|---|
   | 0 | `echo "bridge: no live sessions" >&2; return 1` |
...
5. **Attach.** `tmux attach-session -t "$session"`. No `_bridge_print_last`, no slot allocation, no telegram setup. Pure attach.
...
- **Trailing tab-separated session name** with `-d $'\t' --with-nth=1` — display column is human-formatted, but the exact session name stays attached for unambiguous extraction. Same trick used by the existing meta-search picker at `shell/bridge.sh:1635-1638`.
- **`--height=40% --reverse --prompt=`** — matches existing fzf usage across bridge (`shell/bridge.sh:1637, 1671`).
...
### Refactor: `_bridge_reconcile_slots`

The dead-slot reconciliation block currently inlined in `_bridge_slot_status` at `shell/bridge.sh:1087-1115` (tmux-has-session check, PID-liveness fallback, key pruning) is extracted into its own function. `_bridge_slot_status` calls it; `_bridge_attach_pick` calls it. Behavior is unchanged — the block is idempotent and silent on no-op.
...
A small helper validates the parser-collected flag/positional state before dispatching to `_bridge_attach_pick`. Implemented as a check in `bridge()` *after* the parser loop, so it sees flags that follow `--attach` on the command line:

```bash
# Inside bridge(), after the case-loop and `set -- "${pos[@]}"`:
...
The `--attach` case in the parser loop just sets `mode_attach=1`. A new `mode_attach` local is added to the existing block at `shell/bridge.sh:1458`.
...
| # | Scenario | Expected |
|---|---|---|
| 1 | All slots free | `bridge: no live sessions` on stderr, exit 1 |
...
| 7 | `bridge -a -w doc` / `bridge -a config` | Hard error, exit 2 |
| 8 | Detach (`Ctrl-b d`) from attached session | Returns to shell, `--status` still shows slot occupied |
| 9 | `bridge --at<Tab>`, `bridge -<Tab>` | Completes `--attach`; `-a` listed |
| 10 | `bridge --help` | New `-a, --attach` line present |
...
- **Shared `_bridge_slot_rows` producer** consumed by both `--status` and `--attach` — tempting if more callers appear (e.g. issue #9). YAGNI for now.
