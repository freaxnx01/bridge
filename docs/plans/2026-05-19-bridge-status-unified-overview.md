# bridge `--status` — unified session overview — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Widen `bridge --status` to surface non-slot tmux sessions (`--no-channel`, `--code`, `--opencode`) alongside slot sessions, and merge Remote Control URLs into the same view via a footer. `--status-rc` becomes a deprecated alias.

**Architecture:** All edits land in `bridge.sh`. (1) Extend `_bridge_tmux_session_defaults` to tag each bridge-created tmux session with `@bridge-*` user-options at creation time (no new persistent state file). (2) Rewrite `_bridge_slot_status` to enumerate the union of `slots.json` rows + tmux-tagged sessions, dedup by tmux session name, render a single table, and emit a URLs footer for rows with an active `bridgeSessionId`. (3) Collapse `_bridge_slot_status_rc` to a thin deprecation wrapper. (4) Bump `_BRIDGE_VERSION` (minor) and add a CHANGELOG entry per `CLAUDE.md` convention.

**Tech Stack:** Bash 5, `python3` (already used by `_bridge_slot_status`), `tmux`. All present today.

**Spec:** `docs/specs/2026-05-18-bridge-status-unified-overview-design.md` (issue [#1](https://github.com/freaxnx01/bridge/issues/1))

---

## File Structure

- **Modify:** `bridge.sh` (sole code file touched)
  - Line 25: bump `_BRIDGE_VERSION` from `1.27.0` → `1.28.0` (new feature → minor).
  - Lines 1745–1749: extend `_bridge_tmux_session_defaults` body + signature.
  - Lines 1854, 1885, 1907, 1950: update the four call sites to pass new args.
  - Lines 1271–1313: full rewrite of `_bridge_slot_status`.
  - Lines 1320–1370: collapse `_bridge_slot_status_rc` to alias.
...
The repo has no shell-test harness (no `bats`, no `shunit2`), matching the precedent set by `docs/plans/2026-05-04-bridge-session-picker.md`. Adding one for this change would dwarf the change itself — explicit YAGNI.
...
### Task 1: Extend `_bridge_tmux_session_defaults` to tag sessions

**Files:**
- Modify: `bridge.sh:1745-1749` (`_bridge_tmux_session_defaults` body + signature)

Replace the existing 4-line function body so that, in addition to setting `mouse on` / `history-limit 50000`, it writes `@bridge-*` user-options derived from new positional args.

- [ ] **Step 1: Replace the function**

Find the current definition at `bridge.sh:1745`:
...
- `tmux set-option ... '@bridge-...'` is the standard mechanism for user-defined options in tmux ≥ 3.0. The repo already relies on `set-option` / `set-hook` elsewhere.
- The `>/dev/null 2>&1` redirects match the existing two option-writes and keep the launch path quiet.
- `pid` capture uses the same `tmux display-message ... '#{pane_pid}'` mechanism the slot path already uses at `bridge.sh:1962`. Same value, same semantics.
...
### Task 2: Update the four `_bridge_tmux_session_defaults` call sites

**Files:**
- Modify: `bridge.sh:1854` (copilot path)
- Modify: `bridge.sh:1885` (opencode path)
- Modify: `bridge.sh:1907` (`--no-channel` path)
- Modify: `bridge.sh:1950` (slot path)
...
Find at `bridge.sh:1854`:

```bash
        _bridge_tmux_session_defaults "$session"
```
...
- `$_SLOT` is the slot number set by `_bridge_slot_allocate` earlier in the same code path (`bridge.sh:1918`).
...
SSH_CONNECTION=fake bridge --no-channel bridge &
sleep 2
tmux ls
tmux show-options -t bridge -v '@bridge-repo' 2>/dev/null
tmux show-options -t bridge -v '@bridge-kind' 2>/dev/null
tmux show-options -t bridge -v '@bridge-pid'  2>/dev/null
tmux kill-session -t bridge
```

Expected:
- `tmux ls` shows a `bridge` session.
- `@bridge-repo` prints `bridge`.
- `@bridge-kind` prints `no-channel`.
- `@bridge-pid` prints a non-empty integer pid.
...
### Task 3: Rewrite `_bridge_slot_status` for the unified table + footer

**Files:**
- Modify: `bridge.sh:1271-1313` (`_bridge_slot_status` function)
...
bridge --status-rc > /tmp/bridge-status-rc-before.txt 2>&1
echo "status exit: $?"
wc -l /tmp/bridge-status-before.txt /tmp/bridge-status-rc-before.txt
```
...
- [ ] **Step 2: Replace `_bridge_slot_status`**

Find at `bridge.sh:1271`:
...
tokens = {}
try:
    with open('$_BRIDGE_SLOT_TOKENS') as f: tokens = json.load(f)
except: pass
slots = d.get('slots', {})
...
# can see whether it's active. Slots 1..MAX are bridge-allocated.
...
- **RC for slot 0** — the existing `_bridge_slot_status_rc` reads `~/.claude-s0/sessions/<pid>.json` for slot 0. We keep the same convention here.
...
Per `CLAUDE.md`: any change to `bridge.sh` must ride with a `_BRIDGE_VERSION` bump *and* a matching `CHANGELOG.md` entry in the same commit. The single commit for Tasks 1–6 happens at the end of Task 6.

```bash
git status
```

Expected: `bridge.sh` shown as modified, not yet staged. Move on to Tasks 4–6 without committing.

---

### Task 4: Collapse `_bridge_slot_status_rc` to deprecation alias

**Files:**
- Modify: `bridge.sh:1320-1370` (`_bridge_slot_status_rc` function)

- [ ] **Step 1: Replace the function**

Find at `bridge.sh:1320`:
...
```bash
. bridge.sh
bridge --status-rc 2>/tmp/bridge-deprec.txt
cat /tmp/bridge-deprec.txt
```

Expected:
- stdout: the same unified table as `bridge --status`.
- `/tmp/bridge-deprec.txt` contains: `bridge: --status-rc is deprecated; use --status (RC URLs now shown in the footer)`.

```bash
rm /tmp/bridge-deprec.txt
```
...
**Files:**
- Modify: `bridge.sh:2151-2152` (help heredoc — slot management group)

- [ ] **Step 1: Drop the dedicated `--status-rc` help line**

Find at `bridge.sh:2151`:
...
The old `--status-rc` line is removed. Tab-completion (`bridge.sh:2370`) still lists `--status-rc`, which is the intentional behavior per the spec: typing `--status-rc<TAB>` still completes, then the alias prints the deprecation notice.
...
Per `CLAUDE.md`: version bump and CHANGELOG entry must land in the same commit as the `bridge.sh` changes from Tasks 1–5.

- [ ] **Step 1: Bump version**

Find at `bridge.sh:25`:
...
- `bridge --status` output format: new `KIND`, `TMUX`, and `RC` columns;
...
Expected: two files listed. `bridge.sh` should show diffs near lines 25, 1271, 1320, 1745, 1854, 1885, 1907, 1950, and 2151.

```bash
git commit -m "$(cat <<'EOF'
feat(bridge): unified --status overview surfaces tmux + RC sessions (bump to 1.28.0)

`bridge --status` now lists every bridge-managed Claude session on the
host — slot, --no-channel, --code, --opencode — and merges Remote
Control URLs into a footer. Discovery uses tmux user-options
(@bridge-repo, @bridge-kind, @bridge-pid, etc.) set by
_bridge_tmux_session_defaults at session creation; no new persistent
...
Setup:
1. Start a slot session: `bridge <some-repo>` over SSH (so it lands in tmux). Detach.
2. Start a `--no-channel` session: `bridge --no-channel <other-repo>` over SSH. Detach.
3. Start a hand-rolled tmux session that bypasses bridge: `tmux new-session -d -s manual-test claude`.
...
- The hand-rolled `manual-test` session is **not** in the table (no `@bridge-*` tags).
...
Setup: from the same repo as one of the previous sessions, exit the existing session, then start a worktree-bound session: `bridge <repo> -w feat`. Detach.
...
- For slot sessions: `_bridge_reconcile_slots` flips that slot back to `—` (existing behavior — re-verifies after this change).
...
Setup: kill or detach all bridge sessions.
...
Run:
```bash
bridge --status-rc 2>/tmp/dep.txt
cat /tmp/dep.txt
rm /tmp/dep.txt
```

Expected:
- stdout: identical to `bridge --status`.
- stderr: one line: `bridge: --status-rc is deprecated; use --status (RC URLs now shown in the footer)`.

- [ ] **Scenario 7 — Foreground-mode slot**

Goal: confirm a slot session not running under tmux still shows up correctly.

Setup: from a non-SSH terminal (or with `SSH_CONNECTION` unset), run `bridge <repo>`. This launches `claude` in the foreground; no tmux session is created. Suspend it (Ctrl-Z) or open a second shell.
...
Goal: document — not enforce — that sessions launched by pre-1.28 bridge are invisible to the new table.

Setup (synthetic): manually create a tmux session without setting `@bridge-*` tags: `tmux new-session -d -s old-style claude`. (This simulates a session started by an older bridge where the tagging code didn't exist yet.)
...
If you find a step that references a line number that no longer matches because an earlier task already shifted things, trust the *named* anchor (function name, comment, distinctive string) over the line number. The plan was written against `bridge.sh` at commit `582e15f` (post-spec, pre-implementation).
