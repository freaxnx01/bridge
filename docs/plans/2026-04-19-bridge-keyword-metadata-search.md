# bridge Keyword Metadata Search — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extend `bridge <keyword>` to fall back to searching cached forge metadata (topics + description) when the keyword doesn't match a local repo name.

**Architecture:** Reuse the forge API calls already made by `_bridge_fetch_target` (GitHub/GitLab/Forgejo `list repos` endpoints already return topics + description). Persist them to `~/.cache/bridge/repo-meta.json` alongside the existing `remote.list`. A new `_bridge_meta_search` helper reads that cache and returns matches classified by hit type (topic vs. description). The fallback hooks into the positional-arg branch of `bridge()` after the existing name-match miss.
...
**Modified files:**
- `shell/bridge.sh` — all code changes live here (single file, ~856 lines today):
  - `_bridge_fetch_target` (line 58): also emit description + topics on each line.
  - `_bridge_remote_list` (line 92): write `repo-meta.json` in the same pass it writes `remote.list`.
  - New function `_bridge_meta_search`: read `repo-meta.json`, return matches.
  - `bridge()` positional branch (line ~795): add metadata fallback after name miss.
  - `bridge --refresh` path: also invalidates `repo-meta.json`.
- `shell/BRIDGE.md` — user-facing cheat sheet update.

**New files:**
- None. Cache file `~/.cache/bridge/repo-meta.json` is created at runtime.

---

## Task 1: Capture topics + description in the forge listing

**Why:** All three forge APIs already return topics and description in their bulk listing responses. The existing `_bridge_fetch_target` discards those fields and only keeps `name`. Widen its output so downstream code can cache the metadata.

**Files:**
- Modify: `shell/bridge.sh:58-89` (function `_bridge_fetch_target`)
...
This keeps the stream-to-fzf pipeline in `_bridge_remote_list` working by continuing to feed path-per-line to fzf (first TSV column), while the metadata is captured in parallel.

- [ ] **Step 1: Replace `_bridge_fetch_target` body**

Replace lines 58-89 of `shell/bridge.sh` with:
...
_bridge_fetch_target() {
  ...
  (
    cd "$_BRIDGE_BASE/$rel" 2>/dev/null || exit
...
```bash
source ~/.bash_aliases_bridge 2>/dev/null || source shell/bridge.sh
_bridge_fetch_target github/freaxnx01/public github freaxnx01 public | head -3
```
...
git commit -m "bridge: widen _bridge_fetch_target output to include description and topics"
```

---

## Task 2: Write `repo-meta.json` from `_bridge_remote_list`

**Why:** The widened `_bridge_fetch_target` now emits metadata; persist it to a JSON cache so the search helper can read it quickly without re-hitting APIs.

**Files:**
- Modify: `shell/bridge.sh:92-114` (function `_bridge_remote_list`)
...
Keyed by the same `rel` path bridge uses everywhere else.

- [ ] **Step 1: Replace `_bridge_remote_list` body**

Replace lines 92-114 of `shell/bridge.sh` with:
...
```bash
rm -f ~/.cache/bridge/remote.list ~/.cache/bridge/repo-meta.json
bridge --refresh </dev/null >/dev/null 2>&1 &
sleep 30 && kill %1 2>/dev/null
cat ~/.cache/bridge/repo-meta.json | jq 'to_entries | .[0:3]'
```
...
```bash
git add shell/bridge.sh
git commit -m "bridge: persist forge metadata to repo-meta.json alongside remote.list"
```

---

## Task 3: Add `_bridge_meta_search` helper

**Why:** Pure function that reads `repo-meta.json` and returns matches, classified by hit type. Keeps matching logic out of the main `bridge()` function so it's easy to reason about.

**Files:**
- Modify: `shell/bridge.sh` — add new function between `_bridge_remote_list` (ends ~line 130) and `_bridge_clone_url` (line 116 today).
...
Insert the following after the closing `}` of `_bridge_remote_list` (before `_bridge_clone_url`):
...
_bridge_meta_search claude-dev
```
...
```bash
_bridge_meta_search zzzznomatchzzzz   # should print nothing
_bridge_meta_search dotfiles           # topic hits for whatever repos are tagged
```
...
```bash
git add shell/bridge.sh
git commit -m "bridge: add _bridge_meta_search helper"
```

---

## Task 4: Wire metadata fallback into `bridge()` positional branch

**Why:** The helper is ready. Now hook it in: after an exact-name miss, try `_bridge_meta_search`, handle 0/1/2+ cases.

**Files:**
- Modify: `shell/bridge.sh:794-800` (positional shortcut branch inside `bridge()`)
...
```bash
  ...
  if [ "$mode_delete" = 0 ] && [ -n "$1" ]; then
    ...
    [ -z "$sel" ] && { echo "bridge: no such repo: $1"; return 1; }
    _bridge_launch "$sel" "$worktree"
    return
  fi
```

**Design decision — cloning on 1-hit:**
If the single metadata hit is an uncloned remote (path not present in the local-scan `all`), clone it first via `_bridge_clone_remote` exactly the way the picker path does.
...
```bash
  ...
  if [ "$mode_delete" = 0 ] && [ -n "$1" ]; then
    ...
    if [ "$count" = 1 ]; then
      ...
      _bridge_launch "$hit_path" "$worktree"
      return
    fi
...
Expected: launches Claude in `config` (same outcome as `bridge config`). No picker — single hit auto-launches. Detach/exit.
...
```bash
bridge zzzznomatchzzzz
```

Expected: `bridge: no such repo: zzzznomatchzzzz` on stderr, exit code 1. Same as today.

- [ ] **Step 6: Commit**

```bash
git add shell/bridge.sh
git commit -m "bridge: fall back to metadata search when name match misses"
```
...
**Why:** The existing `--refresh` flag (and `Ctrl-R` in the picker, which invokes `bridge --refresh`) invalidates `remote.list`. `repo-meta.json` needs the same treatment so the next populate overwrites it with fresh data.

**Files:**
- Modify: `shell/bridge.sh` — `_bridge_remote_list` force-refresh branch (already handles it correctly, since Task 2 unconditionally writes `tmp_meta` → `meta_cache` on every non-cached run). Verify no extra change is needed.

- [ ] **Step 1: Audit the refresh path**

Re-read `_bridge_remote_list` after Task 2's edit. The logic is:
...
```bash
ls -la ~/.cache/bridge/remote.list ~/.cache/bridge/repo-meta.json
bridge --refresh </dev/null >/dev/null 2>&1 &
sleep 30 && kill %1 2>/dev/null
ls -la ~/.cache/bridge/remote.list ~/.cache/bridge/repo-meta.json
```
...
**Why:** `BRIDGE.md` is the user-facing cheat sheet. Update it so `bridge <keyword>` behavior is discoverable.

**Files:**
- Modify: `shell/BRIDGE.md`
...
Identify the "Basic usage" / "Surface" section that documents `bridge <name>`.
...
If `bridge <name>` doesn't match a local repo, it falls back to searching
...
```bash
git add shell/BRIDGE.md
git commit -m "docs: document bridge keyword metadata fallback"
```
...
**Why:** bridge is used from three machines (claude-dev LXC, WSL2 on Win11, company notebook). Verify the feature works on each one, since each has different forge credentials in direnv-loaded `.envrc` files.
...
**Spec coverage:** All sections of the spec (`docs/superpowers/specs/2026-04-19-bridge-keyword-metadata-search-design.md`) map to tasks:
- Match flow → Task 4
- Metadata source + cache → Tasks 1, 2
- Refresh triggers (TTL, clone-time opportunistic, Ctrl-R) → Tasks 2, 5. Opportunistic-on-clone is NOT implemented as a separate task — the next `--refresh` picks it up. This is a deliberate scope trim: `_bridge_clone_remote` already invalidates `remote.list`, so the next picker invocation with `-r` will refetch everything. Adding a targeted single-repo metadata refresh is YAGNI.
- Matching rules → Task 3 (`_bridge_meta_search`)
- Picker annotation → Task 4 (awk formatter)
- Changes to `bridge.sh` enumeration → covered across Tasks 1-5.
- Out-of-scope items (local-only repos, no background warming) stay out of scope.
- Success criteria (topic on `config`, `bridge claude-dev` works, existing flows unchanged, no new deps) → Tasks 3, 4, 7.

**Placeholder scan:** No "TBD" / "implement later" / "handle edge cases" in any task. Every code step contains the code to paste.

**Type consistency:** Function names consistent across tasks: `_bridge_fetch_target`, `_bridge_remote_list`, `_bridge_meta_search`, `_bridge_clone_remote`, `_bridge_launch`. Cache file paths consistent: `~/.cache/bridge/remote.list` (existing), `~/.cache/bridge/repo-meta.json` (new). TSV output shape consistent between fetch-target (3 cols: path, desc, topics) and meta-search (3 cols: type, path, snippet) — documented at each function's comment.
