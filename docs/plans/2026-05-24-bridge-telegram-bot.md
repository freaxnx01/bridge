# bridge Telegram wrapper bot — implementation plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Ship a standalone Telegram bot (`bridge-bot`) that wraps `bridge` on claude-dev so the user can spawn new Claude sessions from Telegram via a paginated picker.

**Architecture:** Python 3 stdlib daemon under `systemd --user`. Long-polls Telegram, dispatches to handlers, spawns sessions via `tmux new-session -d 'bash -lc "bridge <name>"'`. No Claude in the command loop. Independent of bot0/admin.

**Tech Stack:** Python 3 (stdlib only — `urllib`, `json`, `subprocess`, `unittest`), `tmux`, `passbolt` CLI, `systemd --user`.

**Spec:** [`docs/specs/2026-05-24-bridge-telegram-bot-design.md`](../specs/2026-05-24-bridge-telegram-bot-design.md)
...
├── bridge_bot.py              # entrypoint: poll loop + dispatcher (executable)
...
├── repos.py                   # local + remote repo list assembly (reads bridge caches)
├── config.py                  # load/save ~/.cache/bridge/bridge-bot.json
└── tests/
    ├── __init__.py
    ├── test_auth.py
    ├── test_picker.py
...
- `setup-claude-channels.sh` — gains a "bridge-bot" section (optional prompt block)
- `bridge-bot/systemd/bridge-bot.service` — shipped in repo, installed via setup script
- `README.md` (top-level) — one paragraph + link to `bridge-bot/README.md`

**Caches read (not owned by this bot):**

- `~/.cache/bridge/slots.json` — read for `/status`, `/kill`, spawn confirmation poll
- `~/.cache/bridge/mru` — read for picker MRU ordering
- `~/.cache/bridge/remote.list` — read when `[🌐 Include remote]` toggled
- `~/projects/repos/` — walked for local repo list

**Caches owned by this bot:**

- `~/.cache/bridge/bridge-bot.json` — config + `last_update_id`
- `~/.cache/bridge/bridge-bot.log` — append-only JSON-line log (written by systemd unit)
- `~/.cache/bridge/bridge-bot.pid` — heartbeat (written every 60s)

---

## Conventions

- **Tests:** Python `unittest` (stdlib). Run with `python3 -m unittest discover bridge-bot/tests -v`.
- **Commits:** Conventional Commits format per `CLAUDE.md`. No `_BRIDGE_VERSION` bump unless we touch `bridge.sh` (we do not in this plan).
...
**Files:**
- Create: `bridge-bot/README.md`
- Create: `bridge-bot/__init__.py` (empty)
- Create: `bridge-bot/tests/__init__.py` (empty)
- Create: `bridge-bot/.gitignore`

- [ ] **Step 1: Create directory layout**

```bash
mkdir -p bridge-bot/tests bridge-bot/systemd
touch bridge-bot/__init__.py bridge-bot/tests/__init__.py
```

- [ ] **Step 2: Write `bridge-bot/README.md`**
...
- [ ] **Step 3: Write `bridge-bot/.gitignore`**
...
```bash
git add bridge-bot
git commit -m "feat(bot): scaffold bridge-bot package layout"
```
...
**Files:**
- Create: `bridge-bot/config.py`
- Test: `bridge-bot/tests/test_config.py`

The config wraps `~/.cache/bridge/bridge-bot.json` with two operations the daemon needs: full load, and atomic update of `last_update_id`.

- [ ] **Step 1: Write failing test**

`bridge-bot/tests/test_config.py`:
...
```bash
python3 -m unittest bridge-bot/tests/test_config.py -v
```

Expected: `ModuleNotFoundError: No module named 'config'`.

- [ ] **Step 3: Implement `bridge-bot/config.py`**

```python
"""Load/save the bridge-bot config file."""

import json
import os
from pathlib import Path

CONFIG_PATH = str(Path.home() / ".cache" / "bridge" / "bridge-bot.json")
...
```bash
git add bridge-bot/config.py bridge-bot/tests/test_config.py
git commit -m "feat(bot): config loader for bridge-bot.json"
```
...
**Files:**
- Create: `bridge-bot/auth.py`
- Test: `bridge-bot/tests/test_auth.py`
...
```bash
python3 -m unittest bridge-bot/tests/test_auth.py -v
```

Expected: `ModuleNotFoundError: No module named 'auth'`.

- [ ] **Step 3: Implement `bridge-bot/auth.py`**
...
```bash
git add bridge-bot/auth.py bridge-bot/tests/test_auth.py
...
**Files:**
- Create: `bridge-bot/repos.py`
- Test: `bridge-bot/tests/test_repos.py`

Two pure functions plus one cache reader. Local list is `walk(REPOS_ROOT)` for `.git` dirs minus prefix; remote list reads `~/.cache/bridge/remote.list`. Ordering uses `~/.cache/bridge/mru` if present.

- [ ] **Step 1: Write failing test**

`bridge-bot/tests/test_repos.py`:
...
```bash
python3 -m unittest bridge-bot/tests/test_repos.py -v
```

Expected: `ModuleNotFoundError: No module named 'repos'`.

- [ ] **Step 3: Implement `bridge-bot/repos.py`**

```python
"""Repo list assembly — reads bridge caches, never writes."""

import os
from pathlib import Path

DEFAULT_ROOT = str(Path.home() / "projects" / "repos")
MRU_PATH = str(Path.home() / ".cache" / "bridge" / "mru")
REMOTE_LIST_PATH = str(Path.home() / ".cache" / "bridge" / "remote.list")
...
```bash
git add bridge-bot/repos.py bridge-bot/tests/test_repos.py
...
**Files:**
- Create: `bridge-bot/picker.py`
- Test: `bridge-bot/tests/test_picker.py`
...
`bridge-bot/tests/test_picker.py`:
...
```bash
python3 -m unittest bridge-bot/tests/test_picker.py -v
```

Expected: `ModuleNotFoundError: No module named 'picker'`.

- [ ] **Step 3: Implement `bridge-bot/picker.py`**
...
```bash
git add bridge-bot/picker.py bridge-bot/tests/test_picker.py
...
- [ ] **Step 1: Implement `bridge-bot/tg.py`**
...
```bash
python3 -c "import sys; sys.path.insert(0, 'bridge-bot'); import tg; print(tg.Bot)"
```
...
**Files:**
- Create: `bridge-bot/spawn.py`
- Test: `bridge-bot/tests/test_spawn.py`
...
`bridge-bot/tests/test_spawn.py`:
...
class SlotPollTests(unittest.TestCase):
    ...
    def test_returns_slot_when_repo_appears(self):
        # Pre-existing slot 1 has "old"; we want to find the new "bridge" slot.
        before = json.load(open(self.tmp.name))["slots"]
        # Simulate bridge writing a new slot:
...
```bash
python3 -m unittest bridge-bot/tests/test_spawn.py -v
```

Expected: `ModuleNotFoundError: No module named 'spawn'`.

- [ ] **Step 3: Implement `bridge-bot/spawn.py`**
...
```bash
git add bridge-bot/spawn.py bridge-bot/tests/test_spawn.py
...
`bridge-bot/tests/test_handlers.py`:
...
class NewCommandTests(unittest.TestCase):
    ...
    def test_new_exact_single_match_spawns_directly(self):
        ...
        self.assertEqual(spawn_calls, [("bridge", "")])
...
```bash
python3 -m unittest bridge-bot/tests/test_handlers.py -v
```

Expected: `ModuleNotFoundError: No module named 'handlers'`.

- [ ] **Step 3: Implement `bridge-bot/handlers.py`**
...
## Task 9: `bridge_bot.py` — entrypoint, poll loop, glue

**Files:**
- Create: `bridge-bot/bridge_bot.py` (executable, shebang `#!/usr/bin/env python3`)
...
- [ ] **Step 1: Implement `bridge-bot/bridge_bot.py`**
...
```bash
chmod +x bridge-bot/bridge_bot.py
```

- [ ] **Step 3: Sanity-check it imports (no Telegram call)**

```bash
python3 -c "import sys; sys.path.insert(0, 'bridge-bot'); import bridge_bot; print(bridge_bot.main)"
```
...
```bash
git add bridge-bot/bridge_bot.py
...
**Files:**
- Create: `bridge-bot/systemd/bridge-bot.service`
...
```bash
git add bridge-bot/systemd/bridge-bot.service
git commit -m "feat(bot): systemd --user unit file"
```

---

## Task 11: Extend `setup-claude-channels.sh` with bridge-bot section
...
Confirm the script ends with the `echo "✓ slot-tokens.json: …"` block and `echo "Done. Run 'bridge --status' to confirm."`.

- [ ] **Step 2: Append the new section**

Edit `setup-claude-channels.sh`. Before the final `echo "Done. Run 'bridge --status' to confirm."` line, insert:
...
```bash
echo "" | bash -c "set -e; export BRIDGE_CACHE=$(mktemp -d); ./setup-claude-channels.sh < <(yes '' | head -20)"
```

This feeds empty answers to all prompts. Expected: script completes without error and the bridge-bot section prints "(skipped — bridge-bot not configured)".

- [ ] **Step 5: Commit**

```bash
git add setup-claude-channels.sh
git commit -m "feat(setup): bridge-bot section in setup-claude-channels.sh"
```
...
```bash
python3 -m unittest discover bridge-bot/tests -v
```
...
In Telegram, DM @BotFather: `/newbot`, give it a name and username (suggested: `claude_freax_bridge_bot`). Store the token in Passbolt; note the resource id.
...
Skip through to the bridge-bot section. Enter the Passbolt resource id and your Telegram user_id. Accept the systemd install.

- [ ] **Step 4: Verify daemon is up**

```bash
systemctl --user status bridge-bot.service
tail -n 20 ~/.cache/bridge/bridge-bot.log
```
...
2. `/status` → output of `bridge --status`.
...
6. Tap `[🔍 Search]` → prompt to reply; reply `bridge` → picker re-renders filtered.
...
From a Telegram account **not** in `allowlist`, DM the bot. Expected: no reply. `~/.cache/bridge/bridge-bot.log` shows `{"evt":"unauthorized","user":…}`.

- [ ] **Step 7: Verify reload**

```bash
systemctl --user reload bridge-bot.service
tail -n 5 ~/.cache/bridge/bridge-bot.log
```
...
**Files:**
- Modify: `README.md` (top-level) — one paragraph under the existing slot/telegram section pointing at `bridge-bot/`
...
```bash
git add README.md
git commit -m "docs: point top-level README at bridge-bot"
```
...
After Task 13, the feature is shipped: a standalone Telegram bot that wraps `bridge` end-to-end, with unit-tested pure pieces, an integration-tested live daemon, an automated setup path, and docs.
...
- Repo deletion
- `/refresh` command
- `bridge --doctor` consumer for the heartbeat
