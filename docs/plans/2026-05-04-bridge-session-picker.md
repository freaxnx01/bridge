# bridge session picker (`--attach`) — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a `bridge -a|--attach` flag that opens an fzf picker over live tmux-backed sessions (the rows `--status` already shows) and reattaches to the selection — zero retyping of repo / worktree.

**Architecture:** All changes live in `shell/bridge.sh`. (1) Extract the dead-slot reconcile block currently inlined in `_bridge_slot_status` into a new `_bridge_reconcile_slots` helper so two callers can share it. (2) Add `_bridge_attach_pick` next to `_bridge_slot_status`: it calls the helper, reads `slots.json`, filters to entries with a non-empty `session` field, and either errors (0 live), auto-attaches (1 live), or runs fzf (2+ live). (3) Add `--attach`/`-a` parsing in `bridge()`, a combo-rejection guard that errors if any other flag/positional was passed, plus help-text and tab-completion updates. No new files, no new state, no new dependencies.

**Tech Stack:** Bash 5, `python3` (already used by `_bridge_slot_status`), `fzf`, `tmux`, `awk`. All present today.

**Spec:** `docs/superpowers/specs/2026-05-04-bridge-session-picker-design.md` (issue [#18](https://github.com/freaxnx01/config/issues/18))

---

## File Structure

- **Modify:** `shell/bridge.sh` (sole source file touched)
  - Line 25: bump `_BRIDGE_VERSION` from `1.15.0` → `1.16.0` (new feature → minor per `CLAUDE.md`).
  - Lines 1082–1150: refactor `_bridge_slot_status` to call the new `_bridge_reconcile_slots`.
  - New function `_bridge_reconcile_slots` inserted just above `_bridge_slot_status` (~line 1080).
  - New function `_bridge_attach_pick` inserted just below `_bridge_slot_status` (~line 1151).
  - Line 1458: add `mode_attach=0` to the existing local-declarations list in `bridge()`.
...
### Task 1: Extract `_bridge_reconcile_slots` helper

**Files:**
- Modify: `shell/bridge.sh:1082-1150` (`_bridge_slot_status` body)
- Insert new function above `_bridge_slot_status` (~line 1080)
...
In a shell where `bridge.sh` is sourced (i.e. your normal interactive shell on `claude-dev`):

```bash
bridge --status > /tmp/bridge-status-before.txt 2>&1
echo "exit: $?"
wc -l /tmp/bridge-status-before.txt
```

Expected: a non-empty file with the current slot table. Note the exit code and line count — the post-refactor run must match.

- [ ] **Step 2: Insert the new `_bridge_reconcile_slots` function**

In `shell/bridge.sh`, immediately above the `_bridge_slot_status` definition (currently `# Print slot status table.` comment at ~line 1081), insert:
...
The python heredoc is a verbatim copy of the block currently inlined at `shell/bridge.sh:1087-1115`, including the `2>/dev/null` swallow. The leading comment is new; nothing else is.

- [ ] **Step 3: Replace the inlined reconcile in `_bridge_slot_status`**

In `shell/bridge.sh`, find the current `_bridge_slot_status` (begins `_bridge_slot_status() {`, currently at line 1082). Replace the body section spanning the comment `# Reconcile dead slots ...` through the closing `" 2>/dev/null` of the first python invocation (lines 1085–1115) with a single call:
...
Only the reconcile python block is removed; the second python block (the table printer) is untouched. Net diff: ~30 lines removed from `_bridge_slot_status`, replaced by one function call.
...
bridge --status > /tmp/bridge-status-after.txt 2>&1
echo "exit: $?"
diff /tmp/bridge-status-before.txt /tmp/bridge-status-after.txt && echo "IDENTICAL"
```
...
```bash
rm -f /tmp/bridge-status-before.txt /tmp/bridge-status-after.txt
```

---

### Task 2: Add `_bridge_attach_pick` function

**Files:**
- Modify: `shell/bridge.sh` — insert new function immediately *after* `_bridge_slot_status` (i.e. between `_bridge_slot_status` and `_bridge_print_last`, at what is currently ~line 1151).

- [ ] **Step 1: Insert `_bridge_attach_pick`**

Insert this function after the closing `}` of `_bridge_slot_status`:
...
- The python block's `2>/dev/null` matches `_bridge_slot_status`'s tolerance for malformed/missing `slots.json` — a python exception swallows silently and `rows` stays empty, falling through to the "no live sessions" path. Edge case 1 in the spec.
- `count` is computed via `grep -c .` so a file with only blank lines also yields 0.
- `key=lambda s: int(s) if s.isdigit() else 999` mirrors the defensive sort in `_bridge_slot_status` — non-numeric keys (which reconcile already drops) sink to the bottom rather than crashing the sort.
...
- No `_bridge_print_last`, no slot allocation, no telegram. Pure attach (spec design step 5).
...
Re-source the script and call the function directly (before the `bridge` flag is wired):
...
- All slots free → `bridge: no live sessions` on stderr, `exit: 1`.
...
### Task 3: Wire `--attach` into `bridge()` parser, help, completion

**Files:**
- Modify: `shell/bridge.sh:1458` (locals list)
- Modify: `shell/bridge.sh` parser case-loop (~line 1461–1525)
- Modify: `shell/bridge.sh` post-parser block (insert after `set -- "${pos[@]}"`, currently line 1526)
- Modify: `shell/bridge.sh` help heredoc (~line 1510, slot-management group)
- Modify: `shell/bridge.sh:1698` (tab-completion `flags=` string)
...
```bash
  local with_remote=0 force_refresh=0 mode_delete=0 worktree="" editor="" remote_control=1 _BRIDGE_NO_CHANNEL=0 _BRIDGE_FORCED_SLOT="" _BRIDGE_NO_SYNC=0
```

Replace with (append `mode_attach=0` at the end):

```bash
  local with_remote=0 force_refresh=0 mode_delete=0 worktree="" editor="" remote_control=1 _BRIDGE_NO_CHANNEL=0 _BRIDGE_FORCED_SLOT="" _BRIDGE_NO_SYNC=0 mode_attach=0
```
...
Insert the dispatch block between `set -- "${pos[@]}"` and `mkdir -p "$_BRIDGE_CACHE"`:
...
```bash
shellcheck -S warning shell/bridge.sh 2>&1 \
  | grep -A2 -E '_bridge_attach_pick|_bridge_reconcile_slots|mode_attach' \
...
### Task 4: Bump `_BRIDGE_VERSION`

**Files:**
- Modify: `shell/bridge.sh:25`

Per `CLAUDE.md`: any change to `shell/bridge.sh` requires a `_BRIDGE_VERSION` bump per semver. This is a new feature → minor bump.
...
```bash
_BRIDGE_VERSION="1.15.0"
```

to:

```bash
_BRIDGE_VERSION="1.16.0"
```
...
This implements the verification table from the spec. Run each scenario in your normal interactive shell on `claude-dev` (where `bridge.sh` is sourced).

Setting up the test fixtures:

- "Live session" = run `bridge <some-repo>` (or `bridge <repo> -w <wt>`) in another terminal/tmux pane, then detach with `Ctrl-b d`. The slot stays allocated.
- "Free slot" = nothing required; default state.
- "Stale slot record" = manually scribble into `~/.cache/bridge/slots.json`: pick a free slot, set its value to a fake record with a non-existent `session` name (e.g. `"session": "ghost_session"`). Reconcile will prune it on the next call.
...
Always `cp ~/.cache/bridge/slots.json /tmp/slots.bak` before editing, and restore at the end. Don't kill the live ones.

- [ ] **Step 1: Scenario 1 — all slots free**

Free any live slots first (`bridge --free N` for each occupied slot), then:
...
Expected:
- stderr: `bridge: no live sessions`
- `exit: 1`
...
```bash
bridge config    # for example, no -w
# Ctrl-b d to detach
```
...
Free the previous session (`bridge --free N`, then `tmux kill-session -t config`). Start a worktree session:
...
```bash
cp ~/.cache/bridge/slots.json /tmp/slots.bak
python3 -c "
import json
p = '/home/freax/.cache/bridge/slots.json'
...
```bash
grep -c ghost ~/.cache/bridge/slots.json
```

Expected: `0`.

If the test went sideways, restore: `cp /tmp/slots.bak ~/.cache/bridge/slots.json`.
...
```bash
cp /tmp/slots.bak ~/.cache/bridge/slots.json
```
...
```bash
bridge -a -w doc;        echo "exit: $?"
bridge --attach config;  echo "exit: $?"
bridge -a -r;            echo "exit: $?"
bridge --attach --no-rc; echo "exit: $?"
bridge -a --slot 3;      echo "exit: $?"
bridge -a -p;            echo "exit: $?"
```

Expected for each: stderr message naming the offending flag/positional, `exit: 2`. Sample expected stderr lines:

```
bridge: --attach takes no other flags (got: -w/--worktree). Run `bridge <repo>` to launch.
bridge: --attach takes no positional args (got: config). Run `bridge <repo>` to launch.
bridge: --attach takes no other flags (got: -r/--remote/--refresh). Run `bridge <repo>` to launch.
bridge: --attach takes no other flags (got: --no-rc). Run `bridge <repo>` to launch.
bridge: --attach takes no other flags (got: --slot). Run `bridge <repo>` to launch.
bridge: --attach takes no other flags (got: -c/-p). Run `bridge <repo>` to launch.
```

Sanity-check the silent-`--rc` carveout (intentional per spec):

```bash
bridge -a --rc; echo "exit: $?"
```
...
With at least one live session: `bridge --attach`, attach, then `Ctrl-b d` to detach. Back at the shell:
...
Expected: completes to `bridge --attach `.
...
```bash
bridge --help | grep -E '^[[:space:]]+-a, --attach'
```
...
```bash
diff /tmp/slots.bak ~/.cache/bridge/slots.json && echo "MATCH" || echo "DIFFERENT — investigate"
```
...
**Files:**
- `shell/bridge.sh` (all changes from Tasks 1–4)
...
2. New `_bridge_reconcile_slots` function inserted above `_bridge_slot_status`.
3. The reconcile python block removed from `_bridge_slot_status`, replaced by a `_bridge_reconcile_slots` call.
4. New `_bridge_attach_pick` function inserted between `_bridge_slot_status` and `_bridge_print_last`.
...
feat(bridge): --attach session picker (closes #18)

Add `bridge -a|--attach` — fzf picker over live, tmux-backed sessions
...
Also extracts the dead-slot reconcile from _bridge_slot_status into a
new _bridge_reconcile_slots helper shared by --status and --attach.

Bumps _BRIDGE_VERSION 1.15.0 -> 1.16.0.
...
Expected: one commit, one file changed (`shell/bridge.sh`), insertions/deletions reflecting the new function + parser changes + version bump.
...
- `_bridge_attach_pick` (reconcile, read & filter, TSV emit, count-branch, attach) → Task 2 Step 1.
...
- `_bridge_reconcile_slots` extraction with byte-identical `--status` → Task 1 Steps 2–3, verified by Task 1 Step 5 diff.
...
- Edge case 1 (`slots.json` missing/unreadable) → covered by `2>/dev/null` in `_bridge_attach_pick`'s python; verified implicitly by Scenario 1.
...
**Type/name consistency:** `_bridge_attach_pick`, `_bridge_reconcile_slots`, `_bridge_slot_status`, `_bridge_slots_init`, `_BRIDGE_SLOTS_FILE`, `_BRIDGE_MAX_SLOTS`, `_BRIDGE_FORCED_SLOT`, `_BRIDGE_NO_CHANNEL`, `_BRIDGE_NO_SYNC`, `mode_attach`, `mode_delete`, `with_remote`, `worktree`, `editor`, `remote_control` all match identifiers already in `shell/bridge.sh` (verified by grep).
