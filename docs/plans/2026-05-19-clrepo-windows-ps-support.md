# clrepo on Windows / PowerShell — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Let Windows users run `clrepo` from PowerShell with `C:\Develop\Repos`-style paths, by keeping `clrepo.sh` canonical and adding a thin `clrepo.ps1` shim that invokes it under Git Bash.

**Architecture:** `clrepo.sh` gains a platform check and a `cygpath`-based path-normalization helper applied once at `_CLREPO_BASE` entry (no-op on POSIX). A new `clrepo.ps1` locates `bash.exe`, sources `clrepo.sh`, forwards arguments, and mirrors the exit code. README documents the Windows install path and the `cd`-doesn't-survive-back-to-PS caveat. ADO is unaffected (already first-class).

**Tech Stack:** Bash 4+, PowerShell 5+, Git for Windows (provides `bash.exe` and `cygpath`).

**Spec:** [`docs/specs/2026-05-19-clrepo-windows-ps-support-design.md`](../specs/2026-05-19-clrepo-windows-ps-support-design.md) (issue #8)
...
These tests assert the behaviour of `_clrepo_is_windows`, `_clrepo_norm_path`, and `_clrepo_display_path` before the helpers exist. The repo has no test framework today; we ship a self-contained Bash assertion script. Run with `bash tests/test_norm_path.sh`; non-zero exit on failure.
...
# Source clrepo.sh in a way that does not auto-run anything. clrepo.sh
...
# --- _clrepo_norm_path: Windows-style on Windows ---
...
git commit -m "test: add failing tests for clrepo path-normalization helpers"
```

---

### Task 2: Implement platform + path helpers in `clrepo.sh`

**Files:**
- Modify: `clrepo.sh` (insert new helpers just above line 36, before `_CLREPO_BASE` is set)

- [ ] **Step 1: Add the three helpers above the `_CLREPO_BASE` assignment.**

Open `clrepo.sh`, find line 36 (`_CLREPO_BASE="${CLREPO_BASE:-$HOME/projects/repos}"`), and insert this block immediately above it (after line 35's `_CLREPO_DIR=...` line):
...
```bash
git add clrepo.sh
git commit -m "feat(clrepo): add platform + path-normalization helpers for Windows"
```

---

### Task 3: Apply normalization to `_CLREPO_BASE` at entry

**Files:**
- Modify: `clrepo.sh:36`

- [ ] **Step 1: Wrap the `_CLREPO_BASE` assignment with normalization.**
...
```bash
unset CLREPO_BASE
bash -c '. ./clrepo.sh && echo "$_CLREPO_BASE"'
```
...
```bash
CLREPO_BASE=/tmp/repos bash -c '. ./clrepo.sh && echo "$_CLREPO_BASE"'
```
Expected: `/tmp/repos`.
...
```bash
git add clrepo.sh
git commit -m "feat(clrepo): normalize CLREPO_BASE for Windows-style inputs"
```

---

### Task 4: Use `_clrepo_display_path` in the prominent user-facing messages

**Files:**
- Modify: `clrepo.sh` lines 1455, 1583, 1643, 2620 (four "under $_CLREPO_BASE" messages)
...
- [ ] **Step 5: Confirm no other `under $_CLREPO_BASE` callsites remain unconverted.**

Run: `grep -n 'under \$_CLREPO_BASE' clrepo.sh`
...
Run:
```bash
CLREPO_BASE=/nonexistent/path bash -c '. ./clrepo.sh && clrepo --list' 2>&1 | head -5
```
Expected: a `clrepo: no ... under /nonexistent/path` style message (the display helper is a no-op on Linux, so the path is unchanged).

- [ ] **Step 7: Commit.**

```bash
git add clrepo.sh
git commit -m "feat(clrepo): display Windows-style base paths in user-facing messages"
```

---

### Task 5: Add the PowerShell shim `clrepo.ps1`
...
```bash
pwsh -NoProfile -Command "Get-Command -Syntax (Resolve-Path ./clrepo.ps1)" 2>&1 | head -5
```
...
```bash
git add clrepo.ps1
git commit -m "feat(clrepo): add PowerShell shim for Windows users"
```
...
**Files:**
- Modify: `clrepo.sh:25` (version bump)
- Modify: `CHANGELOG.md` (new entry)

Per `CLAUDE.md`: every change to `clrepo.sh` bumps `_CLREPO_VERSION` with a matching `CHANGELOG.md` entry. This is a new feature → minor bump: `1.30.0` → `1.31.0`.
...
### Added
- Windows / PowerShell support: `clrepo.ps1` shim invokes `clrepo.sh` under Git Bash, forwarding arguments and exit codes.
- Path-normalization helpers (`_clrepo_norm_path`, `_clrepo_display_path`, `_clrepo_is_windows`) so `CLREPO_BASE=C:\Develop\Repos` and POSIX forms both work.
- Self-contained test for the path helpers at `tests/test_norm_path.sh`.

### Changed
- User-facing "under $_CLREPO_BASE" messages now show Windows-style paths on Windows.
```
...
bash -c '. ./clrepo.sh && echo "$_CLREPO_VERSION"'     # 1.31.0
bash -c '. ./clrepo.sh && echo "$_CLREPO_BASE"'        # your usual base
CLREPO_BASE=/tmp/x bash -c '. ./clrepo.sh && clrepo --list' 2>&1 | head -3
```
...
1. PowerShell, fresh terminal, with `$env:CLREPO_BASE = 'C:\Develop\Repos'` and the appropriate token env vars — confirm `--version`, `--list`, and opening an existing repo all work.
2. Repeat with `$env:CLREPO_BASE = '/c/Develop/Repos'`.
3. From Git Bash on the same box — same results.
4. ADO smoke: an existing ADO repo resolves/clones.
5. Rename `bash.exe` (or set `$env:CLREPO_BASH` to a bogus path) → shim exits 127 with a clear message.
