---
description: List local repos so the user can pick one to open with bridge
allowed-tools: ["Bash"]
...
- Tell the user to open it from a host shell with `bridge $ARGUMENTS`.
  (Launching a Claude session from inside another Claude session via the
  Bash tool would block this current session, so we don't run `bridge`
  directly here.)
