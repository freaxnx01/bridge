...
iter 11 (2026-05-19): PR #18 opened for #5 — `-B`/`--base <dir>` per-invocation override, stacked on PR #17. Pre-pass extracts the flag before main dispatch so it applies to early-return flags too (--status, --pick, --issues, etc.). Accepts `:`-separated lists matching #4's multi-base semantics. Final precedence: flag > env > config file > default. Version 1.37.0. https://github.com/freaxnx01/clrepo/pull/18

Promoting #9 from [~] to [x] in PLAN.md: PR #15 exists and is the Ralph-contract output for #9 (partial-but-real). The 5 unmet acceptance criteria are tracked in the PR description and NOTES as deferred follow-ups for the user to spawn as new issues. With #5 also done, every issue in PLAN has a PR — completion criteria met.
