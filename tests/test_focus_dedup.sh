#!/usr/bin/env bash
# Smoke test for the (forge, owner) dedup in _clrepo_focus_list.
#
...
# `public/` and `private/` .envrc'd subdirs, source clrepo.sh against it,
# then re-apply the dedup awk from `_clrepo_focus_list` to `_clrepo_targets`
...
# dir needs a `.envrc` so `_clrepo_targets` picks it up.
...
# Source clrepo.sh in a subshell pinned to this base.
out=$(
  CLREPO_BASE="$_tmp" \
  bash -c "
    set -u
    . '$_REPO_ROOT/clrepo.sh' >/dev/null 2>&1
    _clrepo_targets | awk -F'\t' '\$2==\"github\" {
      key = \$2 \"\\t\" \$3
...
# resolve to a *single* row even though _clrepo_targets emits two for it.
