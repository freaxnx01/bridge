#!/usr/bin/env bash
# Warn about unpushed commits on bridge session exit.
#
# Two modes:
#   sourced:   defines _bridge_warn_unpushed <repo_path>
#   executed:  $1 = tmux session name; reads stored path, shows tmux display-message.
#
# Same sourcing pattern as bridge-autosync.sh — do NOT enable strict mode at
# file scope (would break the interactive shell that sources bridge.sh).

_BRIDGE_CACHE="${_BRIDGE_CACHE:-$HOME/.cache/bridge}"

_bridge_warn_unpushed() {
  ...
  printf '\033[1;33m⚠  bridge: %d unpushed commit(s) on %s in %s — push before leaving!\033[0m\n' \
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
  ...
  tmux display-message -t "$session" -d 5000 \
    "⚠  bridge: ${count} unpushed commit(s) on ${branch} in ${repo_name} — push before leaving!"
fi
