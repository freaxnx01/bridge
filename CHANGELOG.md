...
- `bridge nav`: the picker's **`r`** key now refetches the repo list from the forge instead of silently re-reading the same `remote.list` cache from disk. Nav never refreshed that cache itself and applied no staleness check, so a renamed or deleted repo could linger as a phantom `↓` row indefinitely (e.g. `↓ github/public/agent-pipeline` long after the rename to `agent-workflow`). **`ctrl+r`** does the same from anywhere in the picker — including the filter box, where the cursor starts and a bare `r` has to stay typeable — and the hint line now advertises `^r refresh`. A failed fetch keeps the previously loaded rows and shows `remote unavailable (cached rows shown)`.
