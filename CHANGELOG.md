...
- `repo-meta.json` is **written again**. The README documented `list -r [--refresh]` as its writer, but no Go code wrote the file — it had been stale since the bash implementation, while `bridge open <keyword>` and `bridge nextgen<TAB>` kept reading it for description/topic search. Every remote refresh (`list -r --refresh`, `bridge --refresh`, nav's `r`/`ctrl+r`) now writes it, keyed by each clone's root-relative path. A repo whose forge fails that round keeps its previous entry rather than losing it, and entries for clones that are gone from disk are dropped. A field the forge does not report (ADO sends no description or topics, GitLab and Forgejo no topics) keeps its cached value instead of being blanked, and a repos root that cannot be enumerated skips the write entirely rather than truncating the file. Entries carry `fetched_at`. `bridge doctor` now points at the real writer when the file is missing. (#297)
...
- Repo pickers (`bridge nav` and the fzf `-r` picker): two **different** GitHub repos that share a name across owners (e.g. `freaxnx01/ai-instructions` and `anim-bossinfo-ch/ai-instructions`) no longer render as an indistinguishable pair of `github/public/ai-instructions` rows that look like a duplicate. The GitHub label drops the owner; when names would otherwise collide, the owner is now injected (`github/<visibility>/<owner>/<name>`) for just the colliding rows, while uniquely-named repos keep the clean owner-less label. (Dedup of an actual local+remote clone is unchanged.)
- Local-only fzf picker (no remote rows): owner disambiguation now also applies here, so two local clones of the same repo name from different owners are distinguishable rather than rendering as identical `github/public/<name>` rows.
