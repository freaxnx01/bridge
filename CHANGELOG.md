...
- `bridge status` now includes a per-slot/session detail table (slot id, kind, repo[wt], agent, age, state, tmux) joining the slot registry with live tmux sessions. Untagged tmux sessions running `claude` outside the bridge launcher appear as `kind=tmux` rows. Restores the at-a-glance view bash `--status` provided (#43). The 4-line summary remains at top; pass `--slim` for summary-only.
