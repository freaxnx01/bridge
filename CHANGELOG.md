...
- `bridge status` now includes a per-slot detail table (slot id, kind, repo[wt], agent, age, state, tmux) joining the slot registry with live tmux sessions. Restores the at-a-glance view bash `--status` provided (#43). The 4-line summary remains at top; pass `--slim` for summary-only. Tmux sessions running outside the slot registry are intentionally excluded — surfacing them would leak unrelated shells / admin sessions; re-adding them with a pane-command filter (e.g. only those running `claude`) is tracked as a follow-up.
